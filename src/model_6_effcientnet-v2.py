...
def train_model(model, train_loader, val_loader, criterion, optimizer, epochs):
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), "efficientnet_v2_best.pth")
    
    return model

def save_model_as_pth(model, filename="efficientnet_v2.pth"):
    torch.save(model.state_dict(), filename)
    print(f"Model saved as {filename}")

def load_model_from_pth(filename, num_classes):
    model = create_efficientnet_v2(num_classes)
...
if __name__ == "__main__":
    ...
    model = create_efficientnet_v2(num_classes)
    print("EfficientNetV2 Architecture:")
    print(model)
